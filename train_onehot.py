...
df = pd.read_csv('~/Desktop/MCM/train_onehot.csv', header=0, sep=',')
#  print(df.head())
...
ans.to_csv('strategy_onehot1.csv', sep=',')
...
for i in range(5,15):
    ...
    y_modified = pd.DataFrame(columns=[i-5], data = rfc.predict(X_modified))
    ans = pd.concat([ans, y_modified], axis=1)
ans.to_csv('strategy_onehot2.csv', sep=',')
...
for i in range(5,15):
    ...
    y_modified = pd.DataFrame(columns=[i-5], data = rfc.predict(X_modified))
    ans = pd.concat([ans, y_modified], axis=1)
ans.to_csv('strategy_onehot3.csv', sep=',')
...
for i in range(5,15):
    ...
    y_modified = pd.DataFrame(columns=[i-5], data = rfc.predict(X_modified))
    ans = pd.concat([ans, y_modified], axis=1)
ans.to_csv('strategy_onehot4.csv', sep=',')
...
for i in range(5,15):
    ...
    y_modified = pd.DataFrame(columns=[i-5], data = rfc.predict(X_modified))
    ans = pd.concat([ans, y_modified], axis=1)
ans.to_csv('strategy_onehot5.csv', sep=',')
...
for i in range(5,15):
    ...
    y_modified = pd.DataFrame(columns=[i-5], data = rfc.predict(X_modified))
    ans = pd.concat([ans, y_modified], axis=1)
ans.to_csv('strategy_onehot6.csv', sep=',')
